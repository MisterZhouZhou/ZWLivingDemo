...
@property (nonatomic, strong) ZWLiveChatViewController * liveChatVC;

@end

@implementation ZWPlayerViewController

- (ZWLiveChatViewController *)liveChatVC {
    
    if (!_liveChatVC) {
        _liveChatVC = [[ZWLiveChatViewController alloc] init];
    }
    return _liveChatVC;
}
...
- (UIButton *)closeBtn {
    ...
    if (!_closeBtn) {
        ...
        _closeBtn.frame = CGRectMake(kScreenWidth - image.size.width - 10, kScreenHeight - image.size.height - 10, image.size.width, image.size.height);
